...
archive under ``docs/tutorials/spatial_extremes/data/cds_insitu_land/``. Once the
...
``docs/tutorials/spatial_extremes/logs/`` so a long historical pull can be watched
...
Credentials (Climate Data Store account + accepted dataset licence):
    ...
    CDSAPI_KEY=<your-key>
exported in the environment, in a ``.env`` file, or in ``~/.cdsapirc``.

Usage (from the xtremax repo root, with ``uv sync --group tutorials``)
-----
    H=docs/tutorials/spatial_extremes/_helpers
    uv run python $H/scripts/fetch_cds_insitu.py                     # 1800-2026
    uv run python $H/scripts/fetch_cds_insitu.py --start 1950-01-01  # narrower
    uv run python $H/scripts/fetch_cds_insitu.py --overwrite         # re-pull all
"""
...
# Make the ``spatial_extremes`` helper package (``_helpers/spatial_extremes``)
# importable; it is tutorial glue, not part of the installed ``xtremax``.
